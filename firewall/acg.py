...
# ----------------------------------------------------------------------------------------
class OBJS(Plurals):
	"""collection of OBJ objects
	:: INSTANCE VARIABLES ::
	obj_grps_list : object groups (OBJ)s in a list
	_repr_dic : object groups (OBJ)s in a dictionary where ‘key’ as group-name, and ‘value’ 
	as its attributes.
	"""
	...
	def get_matching_obj_grps(self, requests):
		"""match provided request members in all object-groups available on device and 
		returns dictionary of object-group names 
		where object-group matches same members in it.
		Input requests formats required: list of full requests dict, containing 'source', 'destination', 'ports' keys.
		"""
		...
		for k, v in candidates.items():
			obj_grps_list = self.matching_obj_grps(v)
			if obj_grps_list:
				group_names[k] = obj_grps_list
		return group_names

	def matching_obj_grps(self, member):
		"""match provided member in all object-groups available on device and 
		returns list of object-group names where object-group matches same members in it.
		Input member formats : string/tuple/list/set.
		"""
...
class OBJ(Singulars):
	...
	:: INSTANCE VARIABLES ::
	obj_grp_name: object group name
	description: description of object group
	_repr_dic: object group details in a dictionary format.

	:: CLASS VARIABLES ::
	valid_member_types

	:: CLASS PROPERTIES ::
	grp_details: object group details in dictionary (helpful in generating copy)
	...
	def __eq__(self, obj): 
		return not len([x for x in self > obj])
	...
	def _add(self, item_type, item): 						### ERRORS THERE, item mandatory,itemtype shold be optional, reevaluate
